...
for i, m in zip(lis, lis1):
  ...
  videos = res.json()['data']['response']['videos']  # .json()以字典的形式呈现网页的内容，对获取的网页内容进行字典取值
  count = 1 # 用于对写入的视频进行计数
  for video in videos:  # 遍历取出每一个视频的url#
    ...
    with open(video_path, 'wb') as file:  # 以二进制的形式保存视频
      print(f'正在写入第{count}/{len(videos)}个视频: {title}')
